...
export class DashboardQueries {
  ...
  async fiveMostExpensiveProducts(): Promise<{name: string, price: number, order_order_id: string}[]> {
    try {
      //@ts-ignore
      const conn = await Client.connect()
      const sql = 'SELECT name, price, order_id FROM products order by price Desc LIMIT 5'
